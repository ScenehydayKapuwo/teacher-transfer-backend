...
exports.requestTransfer = async (req, res) => {
  const { teacherId, toSchoolId, tsno } = req.body;
...
exports.getMyTransferRequests = async (req, res) => {
  try {
    ...
    if (user.role === 'teacher') {
      ...
      whereClause.tsNo = user.tsno;
      console.log(`[INFO] Applying filter for teacher: teacherId=${user.tsno}`);
      ...
      const teacher = await Teacher.findByPk(user.tsno, {
        include: [{ model: School, as: 'currentSchool' }]
      });
