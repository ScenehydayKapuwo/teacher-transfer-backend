...
exports.addProfile = async (req, res) => {
  try {
    ...
    res.status(201).json({
      message: teacherProfile ? "Profile updated successfully" : "User registered successfully",
...
exports.registerUser = async (req, res) => {
  const { username, password, role, tsno } = req.body.form;

  if (!username || !password || !tsno) {
    return res.status(400).json({ message: 'Username, password, and tsno are required.' });
  }

  try {
    const existingUser = await User.findOne({ where: { tsno } });
    if (existingUser) {
